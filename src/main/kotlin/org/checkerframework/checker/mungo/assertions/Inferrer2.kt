...
class Inferrer2(val checker: MungoChecker) {
  ...
  private fun implies(prev: NodeAssertions, next: NodeAssertions, flowRule: Store.FlowRule) {
    when (flowRule) {
      Store.FlowRule.EACH_TO_EACH -> {
        implies(prev.postThen, next.preThen)
        implies(prev.postElse, next.preElse)
      }
      Store.FlowRule.THEN_TO_BOTH -> {
        implies(prev.postThen, next.preThen)
        implies(prev.postThen, next.preElse)
      }
      Store.FlowRule.ELSE_TO_BOTH -> {
        implies(prev.postElse, next.preThen)
        implies(prev.postElse, next.preElse)
      }
  ...
  private fun implies(a: SymbolicAssertion, b: SymbolicAssertion) {
    // TODO
  }
