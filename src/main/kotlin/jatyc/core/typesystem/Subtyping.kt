  ...
  fun isSubtype(a: JTCType, b: JTCType): Boolean {
    return when (a) {
      ...
      is JTCLinearArrayType -> {
        when (b) {
          is JTCUnknownType -> true
          is JTCLinearArrayType -> a.javaType == b.javaType && (b.unknownSize || (!a.unknownSize && a.types.size == b.types.size && a.types.mapIndexed { idx, itA -> isSubtype(itA, b.types[idx]) || (itA is JTCNullType && b.types[idx] !is JTCNullType)}.all { it } ))
          is JTCUnionType -> b.types.any { isSubtype(a, it) }
          is JTCIntersectionType -> b.types.all { isSubtype(a, it) }
